...
export default function SortFinishedTable({ Type }) {
  ...
  React.useEffect(() => {
    if (Filters.hasOwnProperty(Type) || Type === SortType) {
      if (Filters[Type].length > 0 || Type === SortType) {
        setHighLight(true);
      } else {
        setHighLight(false);
      }
  ...
  return (
    <div className="relative inline-flex items-center text-left">
      ...
      <button
        ...
        className={`flex flex-row justify-between items-center w-[2.5rem] text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-md shadow-sm hover:bg-gray-50 ${
      ...
      {/* Dropdown Menu */}
      {isDropdownOpen && (
        <div className="absolute left-0 top-full mt-2 w-[15rem] bg-white border border-gray-300 rounded-md shadow-lg">
          <ul>
            {/* Dropdown Items */}
            {[Type].map((item) => (
              <li
                key={item}
                onClick={() => handleSortClick(item)} // Handles option click
