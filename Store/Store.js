          ...
          if (existingItemIndex !== -1) {
            ...
            const updatedItems = state.data.CommonMLTItems.map((item, index) =>
              index === existingItemIndex
                ? { ...item, Used: item.Used + 1, CreatedDate: timestamp } // Increment "Used" count for the found item
                : item
            );

            updatedItems.sort((a, b) => new Data(b.CreatedDate) - new Date(a.CreatedDate)); // Sort by CreatedDate in descending order
            if (updatedItems.length > 30) {
              updatedItems.splice(30); // Removes all elements after the 30th index
            }
            ...
            const now = new Date();
            const timestamp = now.toISOString(); // Get the current timestamp in ISO format
            const newItem = { ...newData, Used: 1, CreatedDate: timestamp }; // Add the "Used" key with value 1
